    ...
    def run(self):
        ...
        try:
            ...
            if self.stdout is None:
                self.logger.warning("No stdout is set to track")
                # sys.exit(1)
...
class Process(ProcessBase):
    def __init__(
        ...
        **kwdargs
        ...
        Some binaries have special modes to force unbuffered printen (e.g.
...
class Function(ProcessBase):
    ...
    def _run_teardown(self):
        ...
        if self.stdout is not None:
            self.stdout.close_queue()
            del self.stdout
            self.stdout = None
        if self.stderr is not None:
            self.stderr.close_queue()
            del self.stderr
            self.stderr = None
        if self.p is not None:
            del self.p
            self.p = None
        # Force garbage collect for all objects multiprocessing queue and popen objects
        # to be collected to free the semaphores.
        # Otherwise "too many open files" exceptions might be trown
        gc.collect()
